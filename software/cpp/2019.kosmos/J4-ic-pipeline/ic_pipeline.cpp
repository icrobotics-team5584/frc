...
int main( int argc, char *argv[] )
{
  ...
  for (;;)
  {
    ...
    for( size_t i = 0; i< img_filtercontours1->size(); i++ )
     {
       Scalar color = Scalar( rng.uniform(0, 255), rng.uniform(0,255), rng.uniform(0,255) );
       drawContours( img_contours1, *img_filtercontours1, (int)i, color, 2, 8 );
     }
    int contourcount1 = 0;
    int min_x1 = 1000000;
    int max_x1 = 0;
    ...
    cout << "INFO: searching for larget contour in camera 0" << endl;
    for( size_t i = 0; i< img_filtercontours1->size(); i++ ) // iterate through each contour. 
    {
      double a=contourArea( (*img_filtercontours1)[i],false);  //  Find the area of contour
      cout << i << ":" << a << endl;
      if(a>largest_area1)
      {
        largest_area1=a;
    ...
    if( ( contourcount0 != 0 ) && ( contourcount1 != 0 ) )
    {
      ...
      cv::Rect br1 = boundingRect((*img_filtercontours1)[largest_contour_index1]);
