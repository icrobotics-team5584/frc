...
SubDriveBase::SubDriveBase() : Subsystem("SubDriveBase") {
    ...
    differentialDriveTwo = RobotMap::subDriveBaseDifferentialDriveTwo;
...
void SubDriveBase::TakeJoystickInputs( std::shared_ptr<Joystick> stick ) {
	...
	differentialDriveTwo->ArcadeDrive( -stick->GetY() * throttle, -stick->GetX() * throttle );
}

void SubDriveBase::Drive(double power, double direction){
    differentialDriveTwo->ArcadeDrive(power, direction);
}

void SubDriveBase::Stop() {
	differentialDriveTwo->ArcadeDrive(0,0);
}
